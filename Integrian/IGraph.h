...
namespace Integrian
{
	...
	public:
		...
		// Visualization
		// -------------
		RGBColour GetNodeColor(T_NodeType* pNode) const;
		RGBColour GetConnectionColor(T_ConnectionType* pConnection) const;
		float GetNodeRadius(T_NodeType* pNode) const;

		// (Pure) virtuals to be override by specific graph types
		// ----------------------------------------------------
		virtual Point2f GetNodePos(T_NodeType* pNode) const = 0;
		...
		virtual Point2f GetNodeWorldPos(int idx) const { return GetNodePos(idx); }
		Point2f GetNodeWorldPos(T_NodeType* pNode) const { return GetNodeWorldPos(pNode->GetIndex()); }

		virtual int GetNodeIdxAtWorldPos(const Point2f& pos) const = 0;
		T_NodeType* GetNodeAtWorldPos(const Point2f& pos) const { return IsNodeValid(GetNodeIdxAtWorldPos(pos)) ? GetNode(GetNodeIdxAtWorldPos(pos)) : nullptr; }
	...
	protected:
		...
		virtual void OnGraphModified(bool, bool) {}
	...
	inline void IGraph<T_NodeType, T_ConnectionType>::RemoveConnection(int from, int to)
	{
		...
		SafeDelete(conFromTo);
		SafeDelete(conToFrom);
	...
	template<class T_NodeType, class T_ConnectionType>
	inline Integrian::RGBColour IGraph<T_NodeType, T_ConnectionType>::GetNodeColor(T_NodeType* pNode) const
	{
		return RGBColour{};
	}
	...
	template<>
	inline Integrian::RGBColour IGraph<GraphNode2D, GraphConnection2D>::GetNodeColor(GraphNode2D* pNode) const
	{
		return pNode->GetColor();
	}

	template<class T_NodeType, class T_ConnectionType>
	inline Integrian::RGBColour IGraph<T_NodeType, T_ConnectionType>::GetConnectionColor(T_ConnectionType* pNode) const
	{
	...
	// Template specialization
	template<>
	inline Integrian::RGBColour IGraph<GraphNode2D, GraphConnection2D>::GetConnectionColor(GraphConnection2D* pConnection) const
	{
		return pConnection->GetColor();
	}

	template<class T_NodeType, class T_ConnectionType>
	inline float IGraph<T_NodeType, T_ConnectionType>::GetNodeRadius(T_NodeType*) const
	{
		return 1.f;
	}
